...
# File Names
#########################################
output_assignments = 'assignments.csv'  # outputs loan to facility mapping
output_yields = 'yields.csv'            # outputs expected yields of facilities

input_facilities = 'facilities.csv'     # facilities
input_covenants = 'covenants.csv'       # covenants
input_loans = 'loans.csv'               # loans
...
with open(input_facilities,'r') as f:
    reader = csv.reader(f)
    # skip column headers
    next(reader, None)
    for row in reader:
...
with open(input_covenants,'r') as f:
    reader = csv.reader(f)
    # skip column headers
    next(reader, None)
    for row in reader:
...
with open(output_assignments, 'w') as f1:
    fieldNames = ['loan_id', 'facility_id']
    writer = csv.DictWriter(f1, fieldnames = fieldNames)
    writer.writeheader()

    with open(input_loans,'r') as f2:
...
with open(output_yields, 'w') as f:
